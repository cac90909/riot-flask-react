...
function GetPlayerItemsFromId({game_data, item_data}, frame_num, participant_id, frame_list){
    ...
    var player_events = game_data.Events.filter(PertainsToUser).filter(NotTrinket).filter(IsItemRelated)
    ...
    for (var event of player_events){
        ...
        if (event.type == 'ITEM_PURCHASED'){
            ...
            active_items.push(item_name)
        }
        else if (event.type == 'ITEM_DESTROYED'){
            var item_name = SwapItemIdForItemName({item_data}, event.itemId)
            var item_index = active_items.indexOf(item_name)
            if (item_index != -1){
                ...
                if (special_case == false){
                    ...
                    active_items.push(special_item_replacement_name)
    ...
    console.log("Here is the ending transaction sequence:", transaction_sequence)
    console.log("Here is the ending active item list:", active_items)
